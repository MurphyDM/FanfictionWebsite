...
function StoryContainer(props) {
    ...
    React.useEffect(() => {
      if(isContinue) dispatch({type: 'setPageNumber', payload: parseInt(localStorage.getItem(props.newestStories.id)) })
      console.log("CONTINUE DISPATCH", state.pageNumber)      
    }, [isContinue]);

    useBottomScrollListener(()=>{
      console.log('bottom');
      if(props.newestStories) localStorage.setItem(props.newestStories.id, state.pageNumber );
      addPage();
    });
